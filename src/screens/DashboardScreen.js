              ...
              {[
                ...
                >
                  <View style={[styles.quickActionIcon, { backgroundColor: `${action.color}20` }]}>
                    <Icon name={action.icon} size={wp('6%')} color={action.color} />
                  </View>
                  <Text style={[styles.quickActionText, { fontSize: wp('3.5%') }]}>{action.title}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </BaseWidget>
...
const styles = StyleSheet.create({
  ...
  contentContainer: { paddingBottom: hp('2%') },
  widgetsContainer: { paddingHorizontal: wp('2%') },
  quickActions: {
    flexDirection: 'row',
    flexWrap: 'wrap',
